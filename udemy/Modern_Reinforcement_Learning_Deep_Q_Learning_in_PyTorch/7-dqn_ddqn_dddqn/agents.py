...
class Agent:
    def __init__(self, gamma, epsilon, lr, n_actions, input_dims, mem_size, batch_size, eps_min=0.01, eps_dec=5e-7,
                 update_target_freq=1000, algo=None, env_name=None, chkpt_dir='tmp/'):
        self.gamma = gamma
...
class DQNAgent(Agent):
    ...
    def learn(self):
        ...
        q_target_max = self.q_target.forward(new_states_t).max(dim=1)[0]
        q_target_max[dones_t] = 0.0

        q_target = rewards_t + self.gamma * q_target_max
...
class DuelingDQNAgent(Agent):
    ...
    def learn(self):
        ...
        V_target, A_target = self.q_target.forward(new_states_t)

        q_pred = T.add(V_online, (A_online - A_online.mean(dim=1, keepdim=True)))[indices, actions_t]

        q_target_max = T.add(V_target, (A_target - A_target.mean(dim=1, keepdim=True))).max(dim=1)[0]
        q_target_max[dones_t] = 0.0
