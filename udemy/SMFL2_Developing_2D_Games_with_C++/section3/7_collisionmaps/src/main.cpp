...
int main(){
    ...
    loadArrLevel(levelTiles, "tilemap", MAP_N);
    loadArrLevel(levelCollisions, "collmap", MAP_N);
...
void loadArrLevel(int* arrLevel, const string& path, int n){
    string str;
    ifstream ifs(RES_DIR + path + "/" + path + to_string(MAP_N) + ".txt");
